<template>
  <div class="bg-white bg-opacity-10 rounded-2xl p-6 border border-white border-opacity-20 h-full">
    <h3 class="text-xl font-semibold text-white mb-4">Последние действия</h3>
    <div class="space-y-4 overflow-y-auto max-h-[260px] pr-2">
      <div
        v-for="activity in activities"
        :key="activity.id"
        class="flex items-center p-3 bg-white bg-opacity-5 rounded-lg"
      >
        <div
        ...
        <div class="flex-1">
          ...
          <p class="text-blue-100 text-xs">{{ activity.time }}</p>
        </div>
      </div>
    </div>
  </div>
